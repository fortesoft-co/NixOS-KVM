  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      ...
      checks.x86_64-linux = {
        ...
        guest = {
          # Build-time check: generateXML produces well-formed libvirt XML
          # (xmllint --noout) with correct structure (xpath) + the cheap
          # high-value AD on/off value checks. Composed fixtures: one baseGuest
          # + small deltas (AD on/off, with-disks, with-passthrough). Returns a
          # runCommand derivation directly — the build IS the check.
          xml = import ./tests/guest/xml.nix { inherit (pkgs) lib; inherit pkgs; };
        };
      };
    };
}